...
await context.route('**/api/orders**', route => {
  ...
  if (route.request().method() === 'POST') {
    ...
    assert.deepEqual(route.request().postDataJSON(), { items: [{ productId: 1, quantity: 1 }] })
    return json(route, 403, null, '下单请求被拒绝')
  }
  if (route.request().method() === 'PATCH') {
    assert.ok(['Bearer browser-test-token', 'Bearer browser-restored-token'].includes(route.request().headers().authorization))
    assert.equal(new URL(route.request().url()).pathname, '/api/orders/7/cancel')
    fixtureOrder.status = 'CANCELLED'
    return json(route, 200, fixtureOrder)
  }
...
try {
  ...
  await page.getByRole('status').filter({ hasText: '订单 #7 已取消' }).waitFor()
  await page.getByText('已取消', { exact: true }).waitFor()
  assert.equal(await page.getByRole('button', { name: '不可取消', exact: true }).isEnabled(), false)
  console.log('PASS checkout and cancellation use the current Access Bearer; successful cancellation updates the order')
